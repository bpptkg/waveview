...
export const toLatLon = (easting: number, northing: number, zoneNum: number, zoneLetter: string, northern: boolean, strict: boolean) => {
...
export const fromLatLon = (latitude: number, longitude: number, forceZoneNum: number) => {
  ...
  let northing =
    K0 * (m + n * latTan * (a2 / 2 + (a4 / 24) * (5 - latTan2 + 9 * c + 4 * c * c) + (a6 / 720) * (61 - 58 * latTan2 + latTan4 + 600 * c - 330 * E_P2)));
...
export const latitudeToZoneLetter = (latitude: number) => {
...
export const latLonToZoneNumber = (latitude: number, longitude: number) => {
...
export const zoneNumberToCentralLongitude = (zoneNum: number) => {
